...
import {getAllUserReservations, getAllRestaurantReservations, GetRestaurantFromToken} from '../util/APIUtils';
...
class ReservationList extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            userId: GetRestaurantFromToken().restaurantId,
        };
    }
    ...
    loadReservationList() {
        ...
        if (this.props.userId) {
            if (this.props.type === 'user') {
                ...
                promise = getAllRestaurantReservations(this.state.id);
            }
        } else {
            // Jeżeli nie posiada się restaurantId - będzie próbować pobrać z tokena zapisanego w local storage
            promise = getAllRestaurantReservations(this.state.id);
        }
        ...
        promise
            .then(response => {
                // Ustaw listę rezerwacji na response data, lub pustą tablicę w przypadku braku danych.
                this.setState({
                        ...
                        <div key={reservation.id}>
                            ...
                            <Paper>
                                <Typography spacing={2} variant="caption" color="textSecondary">
                                    <Link to={`/user/reservation/${reservation.id}`}>
